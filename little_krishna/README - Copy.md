...
1. Place your target image in the same directory as `particle_animation.py`.
2. Set `IMAGE_PATH` near the top of `particle_animation.py` to the image filename, for example:

```python
IMAGE_PATH = "krishna.png"
```

PNG, JPG, and JPEG images are supported. Only change `IMAGE_PATH` when switching images.
3. Run the script:
