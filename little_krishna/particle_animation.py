...
def load_and_fit(path, w, h):
    ...
    if img is None:
        raise FileNotFoundError(
            f"Image file not found or could not be read: {path}. "
            "Place a PNG, JPG, or JPEG image in the same folder as particle_animation.py "
            "and update IMAGE_PATH."
        )
...
def main():
    try:
        base = load_and_fit(os.path.join(script_dir, IMAGE_PATH), CANVAS_W, CANVAS_H)
